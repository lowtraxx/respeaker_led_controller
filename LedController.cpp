...
bool LedController::InitSpiDevice()
{
	...
	int ioctl_ret_val = 0;
	if((ioctl_ret_val = ioctl(led_spi_file_descriptor_, SPI_IOC_RD_MODE, &spi_mode_)) < 0)
		std::cout << "Failed to get write mode on the LED SPI device" << std::endl;

	// Get bits per word
	if((ioctl_ret_val = ioctl(led_spi_file_descriptor_, SPI_IOC_RD_BITS_PER_WORD, &bits_per_word_)) < 0)
		std::cout << "Failed to get bits per word on the LED SPI device" << std::endl;

	// Get the speed		
	if((ioctl_ret_val = ioctl(led_spi_file_descriptor_, SPI_IOC_RD_MAX_SPEED_HZ, &speed_in_hz_)) < 0)
		std::cout << "Failed to get desired speed on the LED SPI device" << std::endl;

	// Try to set it to fast, so we can update the LEDs faster
	speed_in_hz_ = 8000000;
	if((ioctl_ret_val = ioctl(led_spi_file_descriptor_, SPI_IOC_WR_MAX_SPEED_HZ, &speed_in_hz_)) < 0)
		std::cout << "Failed to set desired speed on the LED SPI device" << std::endl;
...
bool LedController::SetGpioPower(bool power)
{
	...
	memcpy(led_gpio_request.default_values, &data, sizeof(led_gpio_request.default_values));

	// Try to send the power command
	int ioctl_ret_val = 0;
	if((ioctl_ret_val = ioctl(led_gpio_file_descriptor_, GPIO_GET_LINEHANDLE_IOCTL, &led_gpio_request)) < 0)
	{
		std::cout << "Failed to send power " << (power ? " on " : " off ") << "signal to the GPIO" << std::endl;
		return false;
	}
...
void LedController::SetPixelColor(int pixel, uint8_t r, uint8_t g, uint8_t b, uint8_t brightness)
{
	if(!initialized_)
	{
...
void LedController::MakeTransfer(uint8_t *data, int len, int speed_in_hz, int bits_per_word)
{
	...
	int ioctl_ret_val = ioctl(led_spi_file_descriptor_, SPI_IOC_MESSAGE(1), &spi_transfer);
	if(ioctl_ret_val < 0)
		std::cout << "Failed to make data transfer to the SPI LED device" << std::endl;
